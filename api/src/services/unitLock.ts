...
export const UNIT_LOCK_DEFAULT_CONFIG = { limit_kwh: 6 } as const;
...
export function normalizeUnitLockState(value: unknown): UnitLockState {
  ...
  return {
    version: 2,
    limit_kwh: normalizeUnitLockConfig({ limit_kwh: raw.limit_kwh }).limit_kwh,
...
export function planUnitLock(
  ...
  input: UnitLockInput,
  configValue: UnitLockConfig | null = UNIT_LOCK_DEFAULT_CONFIG,
): UnitLockPlan {
